...
2012年AlexNet以8层的深度横扫ImageNet，开启了深度学习的黄金时代。随后VGG将网络推到了19层，GoogLeNet达到了22层。研究人员自然而然地认为：**网络越深，性能越好**——毕竟更深意味着更强的表达能力。

然而，一个令人困惑的现象出现了。

2015年，何恺明等人在论文《Deep Residual Learning for Image Recognition》中报告了一个令人不安的实验结果：在CIFAR-10数据集上，**一个56层的“纯”卷积网络（plain network），其训练误差和测试误差竟然比20层的网络还要高**。

这不是过拟合——过拟合的表现是训练误差低、测试误差高。而56层网络的**训练误差本身就更高**，说明问题出在**优化**上，而不是**泛化**上。

这个现象被称为**退化问题（Degradation Problem）** 。

> **💡 关键洞察**：56层网络的解空间理论上**包含**20层网络的解空间——只要把后36层训练成恒等映射（$y=x$），56层网络就能达到20层的性能。但问题是：**神经网络很难把一堆非线性层训练成“什么都不做”的恒等映射**。

ResNet的贡献正是**让“什么都不做”变得容易**——通过引入**残差块（Residual Block）** ，让网络只需要学习 $F(x)=0$ 而不是 $H(x)=x$。
